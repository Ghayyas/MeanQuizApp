var node_1 = require("../src/path/node");
var module = angular.mock.module;
var rejectFactory_1 = require("../src/transition/rejectFactory");
var common_1 = require("../src/common/common");
var pathFactory_1 = require("../src/path/pathFactory");
var stateMatcher_1 = require("../src/state/stateMatcher");
var stateBuilder_1 = require("../src/state/stateBuilder");
var targetState_1 = require("../src/state/targetState");
var stateQueueManager_1 = require("../src/state/stateQueueManager");
describe('transition', function () {
    ...
    beforeEach(module('ui.router', function ($transitionsProvider, $urlMatcherFactoryProvider) {
        ...
        matcher = new stateMatcher_1.StateMatcher(statesMap = {});
        pathFactory = new pathFactory_1.PathFactory(function () { return root; });
        var builder = new stateBuilder_1.StateBuilder(function () { return root; }, matcher, $urlMatcherFactoryProvider);
        queue = new stateQueueManager_1.StateQueueManager(statesMap, builder, { when: function () { } }, null);
        var root = queue.register({ name: '', url: '^', views: null, 'abstract': true });
        root.navigable = null;
        common_1.forEach(stateTree, function (topLevelState, key) {
        ...
        function registerStates(parent, state, name) {
            var substates = common_1.omit.apply(null, [state].concat(stateProps));
            var thisState = common_1.pick.apply(null, [state].concat(stateProps));
            thisState = common_1.extend(thisState, { name: name, parent: parent });
            queue.register(thisState);
            common_1.forEach(substates, function (value, key) {
    ...
    beforeEach(inject(function ($transitions, $state) {
        ...
        makeTransition = function makeTransition(from, to, options) {
            var fromState = targetState(from).$state();
            var fromPath = pathFactory_1.PathFactory.bindTransNodesToPath(fromState.path.map(function (state) { return new node_1.Node(state); }));
    ...
    describe('provider', function () {
        describe('async event hooks:', function () {
            function PromiseResult(promise) {
                ...
                this.called = function () { return common_1.map(self.get(), function (val, key) { return val !== undefined; }); };
                if (promise)
            ...
            describe('.on()', function () {
                it('should fire matching events when transition starts', inject(function ($transitions, $q) {
                    ...
                    makeTransition("first", "third").run();
                    $q.flush();
                    expect(t).toBeNull();
                    makeTransition("first", "second").run();
                    $q.flush();
            ...
            describe('.onEnter()', function () {
                it('should inject $state$', inject(function ($transitions, $q) {
                    ...
                    makeTransition("", "D").run();
                    $q.flush();
                    expect(common_1.pluck(states, 'name')).toEqual(['A', 'B', 'C', 'D']);
                }));
                it('should be called on only states being entered', inject(function ($transitions, $q) {
                    ...
                    var states = [];
                    makeTransition("B", "D").run();
                    $q.flush();
                    expect(common_1.pluck(states, 'name')).toEqual(['C', 'D']);
                    states = [];
                    makeTransition("H", "D").run();
                    $q.flush();
                    expect(common_1.pluck(states, 'name')).toEqual(['B', 'C', 'D']);
                }));
                it('should be called only when from state matches and the state being enter matches to', inject(function ($transitions, $q) {
                    ...
                    var states = [], states2 = [];
                    makeTransition("A", "D").run();
                    $q.flush();
                    expect(common_1.pluck(states, 'name')).toEqual(['C']);
                    expect(common_1.pluck(states2, 'name')).toEqual([]);
                    states = [];
                    states2 = [];
                    makeTransition("B", "D").run();
                    $q.flush();
                    expect(common_1.pluck(states, 'name')).toEqual(['C']);
                    expect(common_1.pluck(states2, 'name')).toEqual(['C']);
                }));
            });
            describe('.onExit()', function () {
                it('should inject the state being exited as $state$', inject(function ($transitions, $q) {
                    ...
                    var states = [];
                    makeTransition("D", "H").run();
                    $q.flush();
                    expect(common_1.pluck(states, 'name')).toEqual(['D', 'C', 'B']);
                }));
                it('should be called on only states being exited', inject(function ($transitions, $q) {
                    transitionProvider.onExit({ from: "*", to: "*" }, function ($state$) { states.push($state$); });
                    var states = [];
                    makeTransition("D", "B").run();
                    $q.flush();
                    expect(common_1.pluck(states, 'name')).toEqual(['D', 'C']);
                    states = [];
                    makeTransition("H", "D").run();
                    $q.flush();
                    expect(common_1.pluck(states, 'name')).toEqual(['H', 'G']);
                }));
                it('should be called only when the to state matches and the state being exited matches the from state', inject(function ($transitions, $q) {
                    ...
                    makeTransition("D", "B").run();
                    $q.flush();
                    expect(common_1.pluck(states, 'name')).toEqual(['D']);
                    expect(common_1.pluck(states2, 'name')).toEqual([]);
                    states = [];
                    states2 = [];
                    makeTransition("D", "C").run();
                    $q.flush();
                    expect(common_1.pluck(states, 'name')).toEqual(['D']);
                    expect(common_1.pluck(states2, 'name')).toEqual(['D']);
                }));
            });
            describe('.onSuccess()', function () {
                it('should only be called if the transition succeeds', inject(function ($transitions, $q) {
                    ...
                    var states = [];
                    makeTransition("A", "C").run();
                    $q.flush();
                    expect(states).toEqual([]);
                    states = [];
                    makeTransition("B", "C").run();
                    $q.flush();
            ...
            it("return value of 'false' should reject the transition with ABORT status", inject(function ($transitions, $q) {
                ...
                transition.run();
                $q.flush();
                expect(common_1.pluck(states, 'name')).toEqual(['A', 'B', 'C']);
                expect(rejection.type).toEqual(rejectFactory_1.RejectType.ABORTED);
            }));
            it("return value of type Transition should abort the transition with SUPERSEDED status", inject(function ($transitions, $q) {
                ...
                transition.run();
                $q.flush();
                expect(common_1.pluck(states, 'name')).toEqual(['B', 'C']);
                expect(rejection.type).toEqual(rejectFactory_1.RejectType.SUPERSEDED);
            ...
            it("hooks which start a new transition should cause the old transition to be rejected.", inject(function ($transitions, $q) {
                ...
                var states = [], rejection, transition2, transition2success, transition = current = makeTransition("A", "D", { current: currenTransition });
                transitionProvider.onEnter({ from: "*", to: "*" }, function ($state$) { states.push($state$); });
                ...
                $q.flush();
                expect(common_1.pluck(states, 'name')).toEqual(['B', 'C', 'G']);
            ...
            it("hooks which return a promise should resolve the promise before continuing", inject(function ($transitions, $q, $timeout) {
                ...
                resolveDeferredFor("B");
                expect(log.join(';')).toBe("#B;^B;#C");
                resolveDeferredFor("C");
                expect(log.join(';')).toBe("#B;^B;#C;^C;#D");
                resolveDeferredFor("D");
                expect(log.join(';')).toBe("#B;^B;#C;^C;#D;^D;DONE");
            }));
    ...
    describe('Transition() instance', function () {
        describe('.entering', function () {
            it('should return the path elements being entered', inject(function ($transitions) {
                ...
                expect(common_1.pluck(t.entering(), 'name')).toEqual(["A"]);
                t = makeTransition("", "D");
                expect(common_1.pluck(t.entering(), 'name')).toEqual(["A", "B", "C", "D"]);
            }));
            it('should not include already entered elements', inject(function ($transitions) {
                var t = makeTransition("B", "D");
                expect(common_1.pluck(t.entering(), 'name')).toEqual(["C", "D"]);
            }));
        });
        describe('.exiting', function () {
            it('should return the path elements being exited', inject(function ($transitions) {
                ...
                expect(common_1.pluck(t.exiting(), 'name')).toEqual(['D']);
                t = makeTransition("D", "A");
                expect(common_1.pluck(t.exiting(), 'name')).toEqual(["D", "C", "B"]);
            }));
        });
