...
var module_1 = require("../src/resolve/module");
var module_2 = require("../src/state/module");
var module_3 = require("../src/path/module");
var common_1 = require("../src/common/common");
var module = angular.mock.module;
///////////////////////////////////////////////
var states, statesTree, statesMap = {};
var emptyPath;
...
beforeEach(function () {
    ...
    function loadStates(parent, state, name) {
        ...
        thisState = new module_2.State(thisState);
        statesMap[name] = thisState;
        return thisState;
    }
});
function makePath(names) {
    var nodes = common_1.map(names, function (name) { return new module_3.Node(statesMap[name]); });
    return module_3.PathFactory.bindTransNodesToPath(nodes);
}
function getResolvedData(pathContext) {
    return common_1.map(pathContext.getResolvables(), common_1.prop("data"));
}
describe('Resolvables system:', function () {
    ...
    describe('ResolveContext.resolvePathElement()', function () {
        it('should resolve all resolves in a PathElement', inject(function ($q) {
            var path = makePath(["A"]);
            var ctx = new module_1.ResolveContext(path);
            var promise = ctx.resolvePathElement(statesMap["A"]);
            promise.then(function () {
                var expectFn = expect(getResolvedData(ctx));
        ...
        it('should not resolve non-dep parent PathElements', inject(function ($q) {
            ...
            var ctx = new module_1.ResolveContext(path);
            var promise = ctx.resolvePathElement(statesMap["B"]);
            promise.then(function () {
        ...
        it('should resolve only eager resolves when run with "eager" policy', inject(function ($q) {
            ...
            var ctx = new module_1.ResolveContext(path);
            var promise = ctx.resolvePathElement(statesMap["N"], { resolvePolicy: "EAGER" });
            promise.then(function () {
                var results = common_1.map(ctx.getResolvables(), common_1.prop("data"));
        ...
        it('should resolve only eager and lazy resolves in PathElement when run with "lazy" policy', inject(function ($q) {
            var path = makePath(["J", "N"]);
            var ctx = new module_1.ResolveContext(path);
            var promise = ctx.resolvePathElement(statesMap["N"], { resolvePolicy: "LAZY" });
            promise.then(function () {
    ...
    describe('Path.getResolvables', function () {
        it('should return Resolvables from the deepest element and all ancestors', inject(function ($q) {
            ...
            var ctx = new module_1.ResolveContext(path);
            var resolvableLocals = ctx.getResolvables(statesMap["C"]);
    ...
    describe('Path.resolvePath()', function () {
        ...
        it('should resolve only eager resolves when run with "eager" policy', inject(function ($q) {
            var path = makePath(["J", "N"]);
            var ctx = new module_1.ResolveContext(path);
            var promise = ctx.resolvePath({ resolvePolicy: "EAGER" });
            promise.then(function () {
        ...
        it('should resolve only lazy and eager resolves when run with "lazy" policy', inject(function ($q) {
            var path = makePath(["J", "N"]);
            var ctx = new module_1.ResolveContext(path);
            var promise = ctx.resolvePath({ resolvePolicy: "LAZY" });
            promise.then(function () {
                expect(getResolvedData(ctx)).toEqualData({ _J: "J", _N: "JN", _N2: "JN2" });
                asyncCount++;
            });
        ...
        it('should provide each resolveResolvable with a path context, starting from the root to the resolves PathElement', inject(function ($q, $state) {
            ...
            var ctx = new module_1.ResolveContext(path);
            var promise = ctx.resolvePath({ resolvePolicy: "JIT" });
            promise.then(function () {
    ...
    describe('Resolvable.resolveResolvable()', function () {
        it('should resolve one Resolvable, and its deps', inject(function ($q) {
            var path = makePath(["A", "B", "C"]);
            var ctx = new module_1.ResolveContext(path);
            var promise = ctx.getResolvables()["_C"].resolveResolvable(ctx);
            promise.then(function () {
    ...
    describe('PathElement.invokeLater()', function () {
        it('should resolve only the required deps, then inject the fn', inject(function ($q) {
            ...
            var promise = ctx.invokeLater(onEnter1, {});
    ...
    describe('PathElement.invokeLater()', function () {
        it('should resolve the required deps on demand', inject(function ($q) {
            ...
            var promise = ctx.invokeLater(cOnEnter1, {});
            promise.then(function (data) {
                expect(result).toBe("C2");
                expect(getResolvedData(ctx)).toEqualData({ _C2: "C2" });
                asyncCount++;
    ...
    describe('invokeLater', function () {
        it('should Error if the onEnter dependency cannot be injected', inject(function ($q) {
            ...
            var caught;
            var promise = ctx.invokeLater(cOnEnter, {});
    ...
    describe('Resolvables', function () {
        it('should be able to inject deps from the same PathElement', inject(function ($q) {
            ...
            var promise = ctx.invokeLater(dOnEnter, {});
            promise.then(function () {
    ...
    describe('Resolvables', function () {
        it('should allow PathElement to override parent deps Resolvables of the same name', inject(function ($q) {
            ...
            var promise = ctx.invokeLater(fOnEnter, {});
            promise.then(function () {
    ...
    describe('Resolvables', function () {
        it('of a particular name should be injected from the parent PathElements for their own name', inject(function ($q) {
            ...
            var ctx = new module_1.ResolveContext(path);
            var resolvable_G = ctx.getResolvables()["_G"];
            var promise = resolvable_G.get(ctx);
            promise.then(function (data) {
    ...
    describe('Resolvables', function () {
        it('should fail to inject same-name deps to self if no parent PathElement contains the name.', inject(function ($q) {
            ...
            var caught;
            var promise = ctx.invokeLater(iOnEnter, {});
            promise.catch(function (err) {
                caught = err;
                asyncCount++;
            });
    ...
    describe('Resolvables', function () {
        it('should not re-resolve', inject(function ($q) {
            ...
            var promise = ctx.invokeLater(kOnEnter, {});
    ...
    describe('Pre-Resolved Path', function () {
        it('from previous resolve operation should be re-useable when used in another resolve operation', inject(function ($q) {
            var path = makePath(["J", "K"]);
            var ctx1 = new module_1.ResolveContext(path);
            var async = 0;
            ...
            var ctx2 = new module_1.ResolveContext(path2);
...
// Integration tests
describe("Integration: Resolvables system", function () {
    beforeEach(module(function ($stateProvider) {
        var copy = {};
        common_1.forEach(statesMap, function (stateDef, name) {
            copy[name] = common_1.extend({}, stateDef);
        });
        angular.forEach(copy, function (stateDef) {
            if (stateDef.name)
                $stateProvider.state(stateDef);
        });
    }));
    var $state, $rootScope, $transitions, $trace;
    beforeEach(inject(function (_$state_, _$rootScope_, _$transitions_, _$trace_) {
        $state = _$state_;
        $rootScope = _$rootScope_;
        $transitions = _$transitions_;
        $trace = _$trace_;
    }));
    it("should not re-resolve data, when redirecting to a child", function () {
        $transitions.onStart({ to: "J" }, function ($transition$, _J) {
